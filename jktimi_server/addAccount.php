...
if (strnatcmp('iOS', $devicetype) == 0) {
    if($mysqli->errno) {
        echo '服务器端账目添加失败 错误原因: '.$mysqli->error;
    } else {
        echo '服务器端账目添加成功 Success';
    }
} else {
    if($mysqli->errno) {
        echo "<script>alert('Operation Failed With Error: '.$mysqli->error);location='http://2.jktimiserver.applinzi.com/addAccount.html';</script>";
    } else {
