...
if (strnatcmp('iOS', $devicetype) == 0) {
    ...
    /** 针对网页端的响应 */
    if (empty($username1)) {
        echo "<script>alert('用户名不存在,请重新输入');location='http://2.jktimiserver.applinzi.com/login.html';</script>";
    }
    //验证密码
    $result = @$mysqli->query("SELECT password FROM users WHERE username LIKE '$username'");
    $result_arr = @mysqli_fetch_array($result);
    if($result_arr['password'] == $password) {
        echo "<script>alert('登录成功');location='http://2.jktimiserver.applinzi.com/accountBook.php';</script>";
    } else {
        echo "<script>alert('用户名或密码错误,请重新输入');location='http://2.jktimiserver.applinzi.com/login.html';</script>";
    }
}
