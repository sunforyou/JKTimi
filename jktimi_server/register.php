...
if (strnatcmp('iOS', $devicetype) == 0) {
    ...
    if($mysqli->errno) {
        echo '用户注册失败 错误原因: '.$mysqli->error; return;
    }
    echo '用户注册成功 Success';
    ...
    /** 针对网页端的响应 */
    if (!empty($username1)) {
        echo "<script>alert('用户名已存在');location = 'http://2.jktimiserver.applinzi.com/register.html';</script>";
        return;
    }
    //插入数据
    $result = $mysqli->query("INSERT INTO users(username,password) VALUES('$username','$password')");
    ...
    echo "<script>alert('Operation Success');location = 'http://2.jktimiserver.applinzi.com/login.html';</script>";
}
